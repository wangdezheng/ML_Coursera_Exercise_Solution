...
% given input feature vector, compute new feature in hidden layer
X = [ones(m, 1), X]; % 5000 * 401
z2 = Theta1 * X'; % (25 * 401) * (401 *5000)
a2 = sigmoid(z2); % (25 * 5000)

% compute hypothesis in output layer
...
for t=1:m
    ...
    % Step 2  compute delta in output layer
	delta_3 = a3 - y_new(:,t); % (10*1)
	
    z2=[1; z2]; % bias (26*1)

    % Step 3 compute delta in hidden layer
    delta_2 = (Theta2' * delta_3) .* sigmoidGradient(z2); % ((26*10)*(10*1))=(26*1)

    % Step 4 compute gradient in this iteration, add it to total gradient
...
% Step 5 compute final gradient without regularization
